...
def test_derive_exit_rules_cover_not_invited_and_lost_to_winner(tmp_path: Path) -> None:
    observations = clone_payload(canonical_observations_payload())
    ...
    assert "not_invited" in reasons
    assert "lost_to_winner" in reasons
    drop_rows = [
        row
        for row in derivations["analyst_rows"]
...
def test_derive_emits_nda_family_row_for_superseding_agreement(tmp_path: Path) -> None:
    observations = clone_payload(canonical_observations_payload())
    observations["observations"].append(
        {
    ...
    nda_rows = [
        row
        for row in derivations["analyst_rows"]
        if row["analyst_event_type"] == "nda" and row["subject_ref"] == "party_bidder_a"
    ]
    assert any("supersedes:obs_nda" in row["review_flags"] for row in nda_rows)
