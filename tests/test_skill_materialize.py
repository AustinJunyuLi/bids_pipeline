"""Tests for the deterministic materialize stage."""
...
def _evt(
    ...
    *,
    ...
    return {
        ...
        "summary": summary,
        "evidence_refs": [
            {"block_id": block_id, "evidence_id": None, "anchor_text": "x"}
        ],
        "terms": None,
...
def test_dedup_collapses_same_type_date_actor_block(tmp_path: Path) -> None:
    events = [
        ...
        _evt(
            "evt_002", "final_round_ann", block_id="B036", summary="longer description"
        ),
        _evt("evt_003", "executed", actor_ids=["a"], date="2016-07-13"),
    ]
    _write_canon_fixture(tmp_path, events=events)
    run_materialize("imprivata", project_root=tmp_path)
    paths = build_skill_paths("imprivata", project_root=tmp_path)
    result = json.loads(paths.materialized_events_path.read_text(encoding="utf-8"))
...
def test_recover_unnamed_party_from_count_gap(tmp_path: Path) -> None:
                ...
                "evidence_refs": [
                    {
                        "block_id": "B030",
                        "evidence_id": None,
                        "anchor_text": "two financial sponsors",
                    }
                ],
            },
        ],
    ...
    raw_actors_before = json.loads(
        (
            tmp_path / "data" / "skill" / "imprivata" / "extract" / "actors_raw.json"
        ).read_text(encoding="utf-8")
    )
    raw_events_before = json.loads(
        (
    ...
    actors_result = json.loads(
        paths.materialized_actors_path.read_text(encoding="utf-8")
    )
    events_result = json.loads(
        paths.materialized_events_path.read_text(encoding="utf-8")
    )
    log = json.loads(paths.materialize_log_path.read_text(encoding="utf-8"))
    raw_actors_after = json.loads(paths.actors_raw_path.read_text(encoding="utf-8"))
    raw_events_after = json.loads(paths.events_raw_path.read_text(encoding="utf-8"))

    # Placeholder actor created
    assert len(actors_result["actors"]) == 2
    placeholder = [
        a for a in actors_result["actors"] if a["actor_id"].startswith("placeholder_")
    ]
    ...
    assert raw_actors_after == raw_actors_before
    assert raw_events_after == raw_events_before


def test_recover_unnamed_party_fail_closed_no_unresolved_mention(
    tmp_path: Path,
) -> None:
    actors_payload = {
        "actors": [
