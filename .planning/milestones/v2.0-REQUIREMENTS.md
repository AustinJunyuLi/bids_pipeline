...
- [ ] **DERIVE-05**: Graph-linked `AnalystRowRecord` compilation from observations + derived records with `subject_ref`, `row_count`, date/terms fields, and provenance; benchmark-expanded anonymous slot emission remains export-only
...
- [ ] **EXPORT-02**: Triple export surface: `literal_observations.csv`, `analyst_rows.csv`, `benchmark_rows_expanded.csv`, with optional synthetic anonymous slot expansion only in the benchmark-expanded export
