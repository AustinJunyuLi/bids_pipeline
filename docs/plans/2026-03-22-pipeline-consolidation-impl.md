...
> Historical note: this plan documents the migration path, not the final operator contract. The live runtime is defined in `CLAUDE.md`; compatibility-only surfaces such as `deal-agent`, `source-discover`, `canonicalize`, `canonicalize_dir`, and `canonicalize_log_path` were removed in the final production layout.

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Consolidate the production pipeline into Python modules in
`skill_pipeline/`, producing a single `skill-pipeline run --deal <slug>`
command for production execution while keeping benchmark reconciliation as a
standalone post-production diagnostic script.
...
def run_deal(
    ...
    *,
    ...
    print(f"[1/10] Extracting actors and events...")
    run_extract(deal_slug, project_root=project_root)

    print(f"[2/10] Materializing canonical artifacts...")
    run_materialize(deal_slug, project_root=project_root)

    print(f"[3/10] Running structural checks...")
    run_check(deal_slug, project_root=project_root)

    print(f"[4/10] Running quote verification...")
    run_verify(deal_slug, project_root=project_root)

    print(f"[5/10] Running coverage audit...")
    run_coverage(deal_slug, project_root=project_root)

    print(f"[6/10] Running omission audit...")
    run_omission_audit(deal_slug, project_root=project_root)

    print(f"[7/10] Running repair loop...")
    run_repair(deal_slug, project_root=project_root)

    print(f"[8/10] Running deterministic enrichment...")
    run_enrich_core(deal_slug, project_root=project_root)

    print(f"[9/10] Running interpretive enrichment...")
    run_enrich_interpret(deal_slug, project_root=project_root)

    print(f"[10/10] Exporting CSV...")
...
**CLI:** Add `run` subparser with `--deal`.
...
## Phase 9: Standalone Reconcile

### Task 11: Keep reconciliation outside the pipeline

**Status:** Superseded by explicit product decision.

Reconciliation must remain outside `skill-pipeline` and outside
`skill_pipeline/run.py`. It is a standalone, post-production diagnostic script
that reads exported pipeline artifacts plus Alex's workbook.

**Files:**
- Create: `scripts/reconcile_alex.py`
- Create: `tests/test_reconcile_alex_script.py`
- Modify: `CLAUDE.md`

**Contract:**
- No `skill_pipeline/reconcile.py`
- No `skill-pipeline reconcile` subcommand
- Invoke only after `skill-pipeline run --deal <slug>` completes
- Writes `data/skill/<slug>/reconcile/alex_rows.json`
- Writes `data/skill/<slug>/reconcile/reconciliation_report.json`

---

## Phase 10: Cleanup

### Task 12: Delete legacy instruction mirrors, remove stale residue, update docs

**Files:**
- Delete: `.claude/skills/` legacy skill files
- Delete: `.codex/skills/` (entire directory)
- Delete: `.cursor/skills/` (entire directory)
- Delete: `quality_reports/session_logs/2026-03-18_skill-pipeline-design.md`
- Delete: `data/runs/pipeline_state.sqlite`
- Modify: `CLAUDE.md`
- Modify: `docs/design.md`

**Step 1: Delete legacy mirrors and stale residue**

```bash
git rm -r .claude/skills .codex/skills .cursor/skills
git rm quality_reports/session_logs/2026-03-18_skill-pipeline-design.md
git rm data/runs/pipeline_state.sqlite
```

**Step 2: Update CLAUDE.md and active docs**
...
- Remove all `/extract-deal`, `/verify-extraction`, `/enrich-deal`,
  `/export-csv` references from the production flow
- Keep reconciliation documented as
  `python scripts/reconcile_alex.py --deal <slug>` after production export
...
- Add doc-precedence language so archival plans/specs cannot override the live contract
...
- Run `python scripts/reconcile_alex.py --deal stec` and check status
...
Compare output against existing saks artifacts from prior runs, then optionally
run `python scripts/reconcile_alex.py --deal saks`.
...
Task 4 (llm.py) ──────────────────────────→ │
                                   ...
                                   Task 9 (enrich_interpret.py)
                                            ...
                                            ↓
                                   Task 10 (run.py)
                                   Task 11 (standalone reconcile script)
                                            │
                                            ↓
                                   Task 12 (cleanup)
                                            │
