...
from skill_pipeline.schemas.raw import RawDiscoveryManifest
from skill_pipeline.schemas.source import FilingCandidate, SeedDeal
...
def _extract_unambiguous_accession(url: str | None) -> str:
    if not url:
        raise ValueError(
            "primary_url is required and must be a standard SEC Archives filing URL"
        )
