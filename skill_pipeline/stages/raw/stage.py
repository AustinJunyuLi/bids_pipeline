...
from skill_pipeline.core.config import RAW_DIR
from skill_pipeline.schemas.raw import RawDocumentRegistry
from skill_pipeline.schemas.source import SeedDeal
from skill_pipeline.stages.raw.discover import (
    build_raw_discovery_manifest,
    extract_cik_from_url,
)
from skill_pipeline.stages.raw.fetch import (
    atomic_write_json,
...
def fetch_raw_deal(
    ...
    *,
    ...
    atomic_write_json(
        raw_deal_dir / "discovery.json", discovery.model_dump(mode="json")
    )
    ...
    registry = RawDocumentRegistry(
        run_id=run_id, deal_slug=seed.deal_slug, documents=[document]
    )
    atomic_write_json(
        raw_deal_dir / "document_registry.json", registry.model_dump(mode="json")
    )
    return {
        "deal_slug": seed.deal_slug,
        "cik": discovery.cik,
        ...
        "frozen_count": 1,
        "discovery_path": str(
            (raw_deal_dir / "discovery.json").relative_to(raw_dir.parent)
        ),
