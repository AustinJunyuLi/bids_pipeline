"""Tiny deterministic structural gate for materialized skill artifacts."""
...
def _check_proposal_terms(artifacts: LoadedArtifacts) -> list[CheckFinding]:
...
def _check_bidder_kind(artifacts: LoadedArtifacts) -> list[CheckFinding]:
    findings: list[CheckFinding] = []
...
def _check_empty_anchor_text(artifacts: LoadedArtifacts) -> list[CheckFinding]:
    findings: list[CheckFinding] = []
...
def run_check(deal_slug: str, *, project_root: Path = PROJECT_ROOT) -> int:
    """Run structural checks on materialized skill artifacts.

    Reads materialize/ actors, events, and spans; writes check_report.json.
