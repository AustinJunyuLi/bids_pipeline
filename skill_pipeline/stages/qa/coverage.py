"""Deterministic coverage audit over source evidence and materialized artifacts."""
...
def _build_coverage_cues(
    ...
    for item in evidence_items:
        block_ids = _block_ids_for_evidence(
            item, blocks_by_document.get(item.document_id, [])
        )
        if not block_ids:
            continue
...
def _block_ids_for_evidence(
    item: EvidenceItem, blocks: list[ChronologyBlock]
) -> list[str]:
    block_ids: list[str] = []
    for block in blocks:
        overlaps = not (
            block.end_line < item.start_line or block.start_line > item.end_line
        )
...
def _cue_is_covered(cue: CoverageCue, artifacts: LoadedArtifacts) -> bool:
    ...
    for event in artifacts.events.events:
        if (
            cue.suggested_event_types
            and event.event_type not in cue.suggested_event_types
        ):
            continue
...
def _build_findings(
    cues: list[CoverageCue], artifacts: LoadedArtifacts
