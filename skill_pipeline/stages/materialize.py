"""Deterministic materialization: schema upgrade, dedup, NDA-gate, unnamed-party recovery."""
...
from skill_pipeline.core.loaders import (
    ...
    for event in events_payload["events"]:
        ...
        if event.get("deadline_date") is not None:
            event["deadline_date"] = _convert_raw_date(
                event["deadline_date"],
                anchor_date=ResolvedDate.model_validate(event["date"])
                if event.get("date")
                else previous_resolved_date,
...
def _recover_unnamed_parties(
    ...
    for kind, subject in _KIND_SUBJECTS.items():
        ...
        matching_mentions = [
            ...
            and (
                "sponsor" in mention.lower()
                or "buyer" in mention.lower()
                or "bidder" in mention.lower()
            )
        ]
...
def run_materialize(deal_slug: str, *, project_root: Path = PROJECT_ROOT) -> int:
    """Upgrade raw extract artifacts to canonical provenance in materialize/."""
    ...
    if not paths.document_registry_path.exists():
        raise FileNotFoundError(
            f"Missing required input: {paths.document_registry_path}"
        )
    ...
    document_lines = load_document_lines(paths.raw_root / deal_slug / "filings")
    ...
    actors_dict, new_events, recovery_log = _recover_unnamed_parties(
        actors_dict, events
    )
