...
**Date:** 2026-03-22
**Status:** Approved
**Goal:** Consolidate all agent-skill-driven stages into deterministic Python
modules within `skill_pipeline/`, replacing the hybrid agent+CLI workflow with
a single `skill-pipeline run --deal <slug>` command.
...
skill-pipeline run --deal <slug>
    ...
    +-- 11. reconcile        [opt]   -> reconcile/reconciliation_report.json
```
...
client = OpenAI(
    ...
    base_url=os.environ.get("NEWAPI_BASE_URL", "https://api.newapi.pro/v1"),
)
...
NEWAPI_BASE_URL        -- default: https://api.newapi.pro/v1
...
End-to-end orchestrator. Executes stages 1-11 in sequence.

```python
def run_deal(deal_slug: str, *, project_root: Path, skip_reconcile: bool = False) -> int:
...
| `reconcile/` | reconcile | `alex_rows.json`, `reconciliation_report.json` |
...
skill-pipeline reconcile --deal <slug>        # post-export benchmark QA
...
- Agent skill SKILL.md files from `.claude/skills/` (move to docs/ first)

### Move to docs
- `.claude/skills/*/SKILL.md` -> `docs/skills/*.md` (reference only)
...
5. Run reconcile on both deals to verify benchmark alignment
