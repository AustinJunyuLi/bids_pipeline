...
def _write_materialized_artifacts(
    ...
    *,
            ...
            spans.append(
                {
                    ...
                    "evidence_ids": [ref["evidence_id"]] if ref.get("evidence_id") else [],
    ...
    for event in events_payload.get("events", []):
        ...
        event_copy["deadline_date"] = _resolved_date_from_hint(event_copy.get("deadline_date"))
        canonical_events["events"].append(event_copy)

    (materialize_dir / "actors.json").write_text(json.dumps(canonical_actors), encoding="utf-8")
    (materialize_dir / "events.json").write_text(json.dumps(canonical_events), encoding="utf-8")
    (materialize_dir / "spans.json").write_text(json.dumps({"spans": spans}), encoding="utf-8")
