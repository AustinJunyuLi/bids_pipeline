...
def test_recover_unnamed_party_from_count_gap(tmp_path: Path) -> None:
    ...
    raw_actors_before = json.loads((tmp_path / "data" / "skill" / "imprivata" / "extract" / "actors_raw.json").read_text(encoding="utf-8"))
    raw_events_before = json.loads((tmp_path / "data" / "skill" / "imprivata" / "extract" / "events_raw.json").read_text(encoding="utf-8"))
    run_materialize("imprivata", project_root=tmp_path)
    paths = build_skill_paths("imprivata", project_root=tmp_path)

    actors_result = json.loads(paths.materialized_actors_path.read_text(encoding="utf-8"))
    events_result = json.loads(paths.materialized_events_path.read_text(encoding="utf-8"))
