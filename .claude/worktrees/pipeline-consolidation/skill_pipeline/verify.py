...
from skill_pipeline.extract_artifacts import LoadedExtractArtifacts, load_materialized_artifacts
...
def run_verify(deal_slug: str, *, project_root: Path = PROJECT_ROOT) -> int:
    ...
    if not paths.materialized_actors_path.exists():
        raise FileNotFoundError(f"Missing required input: {paths.materialized_actors_path}")
    if not paths.materialized_events_path.exists():
        raise FileNotFoundError(f"Missing required input: {paths.materialized_events_path}")
